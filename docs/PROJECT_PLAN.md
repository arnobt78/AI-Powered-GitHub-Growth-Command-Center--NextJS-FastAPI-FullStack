...
| TBD | VPS + Vercel deployment — blocked on Product Owner action: `alembic upgrade head` against production Postgres (5 pending migrations), generating `RECORDING_AUTH_SECRET` (same value in both `backend/.env` and `frontend/.env.local`), installing `ffmpeg` + running `playwright install chromium --with-deps` on the deploy target, then the Coolify/Vercel deploy itself (POL-0006 Human Gate 2) |
| TBD | Phase 3 (visual/portfolio polish), and 4C/4D's deferred cross-posting/Reddit scope + 4G's deferred cloud-storage scope, once the relevant developer apps/provider accounts are registered |
