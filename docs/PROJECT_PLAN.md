...
| TBD | VPS + Vercel deployment — same `RECORDING_AUTH_SECRET` value, `ffmpeg`, and Playwright chromium (`--with-deps` for the Linux container) still need installing on the actual deploy target, and `alembic upgrade head` still needs running against production Postgres — none of this is reachable from a local dev machine |
