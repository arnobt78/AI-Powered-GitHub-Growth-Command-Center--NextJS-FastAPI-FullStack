...
def test_mint_produces_token_with_payload_and_signature_segments():
    token = mint_recording_token(repo_id=42, user_id=7, github_id="12345")
...
def test_mint_produces_different_signature_for_different_repo():
    token_a = mint_recording_token(repo_id=1, user_id=7, github_id="12345")
    token_b = mint_recording_token(repo_id=2, user_id=7, github_id="12345")
...
def test_mint_refuses_to_produce_a_token_when_secret_is_not_configured(monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        recording_auth_module.mint_recording_token(repo_id=1, user_id=1, github_id="12345")
